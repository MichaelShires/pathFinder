...
int pathFind(struct Route route)
{
...
struct Route initRoute(int startLoc, int endLoc) {
	struct Route blankRoute;
...
void findFastestPath(struct Route route) {
	pathFind(route);
	//cout << route.initStart << ',' << route.endLoc << ',' << pDist << ',' << pStep << ',';
	printf("%d,%d,%d,%d", route.initStart, route.endLoc, pDist, pStep);
...
int main(int argc, char* argv[]) {
	...
	struct Route route = initRoute(initStart, endLoc);
